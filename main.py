...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def updatePreview(self, sprite_path):
        """Load a sprite preview robustly, even when Qt's image plugins fail."""
        if not sprite_path:
            self.spritePreview.clear()
            return

        full_path = os.path.normpath(os.path.join(spriteHandler.basepath, sprite_path))
        if not os.path.isfile(full_path):
            self.spritePreview.clear()
            self.infoBox.appendPlainText(f"Preview image not found: {full_path}")
            return

        try:
            # Use Pillow as a fallback/primary decoder. This avoids failures from
            # missing or incompatible Qt image-format plugins in packaged builds.
            ...
            self.spritePreview.clear()
            self.infoBox.appendPlainText(f"Preview failed: {exc}")
    ...
    def updatePreview(self, current, previous):
        if current is not None:
            full_path = os.path.normpath(
                os.path.join(spriteHandler.basepath, str(current.text()))
            )
            if not os.path.isfile(full_path):
                self.preview.clear()
                return
