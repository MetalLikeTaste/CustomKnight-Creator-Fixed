...
class spriteHandler:
    ...
    @staticmethod
    def fullSpritePath(sprite_path):
        """Return a normalized absolute path for a sprite entry."""
        return os.path.normpath(os.path.join(spriteHandler.basepath, sprite_path))
        ...
        for j in range(0, len(spriteCollectionList)):
            if spriteHandler.categories[spriteCollectionList[j]]:
                ...
                for i in range(0, len(spriteHandler.spriteIDs)):
                    if spriteHandler.spriteCollection[i] == spriteCollectionList[j]:
                        im = Image.open(
                            spriteHandler.fullSpritePath(spriteHandler.spritePath[i])
                        )
                        im = im.crop(
                            (
    ...
    def copyMain(main):
        ...
        mainImage = Image.open(spriteHandler.fullSpritePath(main))
        ...
        for image in groupOfDuplicates:
            ...
            duplicateImage = Image.open(spriteHandler.fullSpritePath(image))
            ...
            duplicateImage.save(spriteHandler.fullSpritePath(image))
    ...
    def checkCompletion(duplicates, vanillaHash):
        ...
        for sprite in duplicates:
            ...
            im = Image.open(spriteHandler.fullSpritePath(spriteHandler.spritePath[i]))
            im = im.crop(
                (
                    spriteHandler.spriteXR[i],
